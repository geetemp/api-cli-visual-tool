...
(function($) {
  ...
  function loadData(data, keyField) {
    ...
    for (
      var row = startRow - 1;
      row < (base.currentPageIdx + 1) * base.pageSize && row < data.length;
      ++row
    ) {
      // start a new row
      ...
      for (; col < base.headers.length; ++col) {
        ...
        if (
          base.headers[col].renderIf != null &&
          !base.headers[col].renderIf(data[row])
        ) {
          cell.append("&nbsp;");
          rowStr.append(cell);
          continue;
        ...
        if (
          base.headers[col].icons != null &&
          base.headers[col].icons.length > 0
        ) {
          jQuery.each(base.headers[col].icons, function(idx, ic) {
            ...
            if (ic.handler != null)
              cell.on(
                ...
                {
                  handler: ic.handler,
                  data: { rowIdx: row, id: data[row][keyField] }
                },
                doIconClick
              );
        ...
        if (fieldName != null) {
          ...
          else cell.append(data[row][fieldName]);
        }
      ...
      rowStr.on(
        "click",
        null,
        { rowIdx: row, id: data[row][keyField] },
        doRowClick
      );
  ...
  function renderTable() {
    ...
    var skel = jQuery(
      '<table class="table table-striped table-bordered" style="margin-bottom:0px;"><thead><tr></tr></thead><tbody></tbody></table>'
    );
    ...
    jQuery.each(base.headers, function(idx, h) {
      ...
      if (h.width != undefined) cell.css("width", h.width + "px");

      if (h.sort) {
        var sortBtn = jQuery(
          "<span class='glyphicon glyphicon-sort-by-attributes' style='color:LightGray' aria-hidden='true' />"
        );
        cell.append("&nbsp;");
        cell.append(sortBtn);
    ...
    base.parentElem.append(
      '<div id="raytable-footer" style="padding:5px;"><span id="raytable-footer-summary" style="float:right;">0 - 0 of 0 items</span></div>'
    );
  }
  ...
  function renderFooter(params) {
    ...
    // update the summary
    base.parentElem
      .find("#raytable-footer-summary")
      .text(
        params.start + " - " + params.end + " of " + params.total + " items"
      );

    // render pagination control
    var pager = jQuery(
      '<ul class="pagination pagination-sm" id="raytable-footer-pager" style="margin-top:0px;"></ul>'
    );
    ...
    if (params.total > base.pageSize) {
      ...
      if (base.maxPageButtons < 2) base.maxPageButtons = 2;

      if (base.maxPageButtons > maxPage) base.maxPageButtons = maxPage;
      ...
      var first = jQuery(
        '<li><a href="#" data="0" aria-label="Previous">&laquo;</a></li>'
      );
      ...
      for (
        page = startPage;
        page < startPage + base.maxPageButtons &&
        page * base.pageSize < params.total;
        ++page
      ) {
        var li =
          page == base.currentPageIdx
            ? jQuery('<li class="active"></li>')
            : jQuery("<li></li>");
        var anchor = jQuery(
    ...
    var summary =
      '<span id="raytable-footer-summary" style="float:right;">' +
      params.start +
      " - " +
      params.end +
      " of " +
